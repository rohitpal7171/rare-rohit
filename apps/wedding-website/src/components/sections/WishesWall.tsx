...
import { useWishes, type Wish } from '@shared/hooks'
...
const WishCard = ({ wish }: { wish: Wish }) => {
  ...
  return (
    <motion.div variants={scaleIn} layout className="card-wish space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-body text-sm font-semibold text-maroon">{wish.name}</p>
            <p className="font-body text-xs text-maroon/50">{dateStr}</p>
          </div>
        </div>
      ...
      <p className="font-body text-sm leading-relaxed text-maroon/70" lang={wish.language}>
        {wish.message}
      </p>
    </motion.div>
  )
    ...
    >
      {submitted ? (
        ...
        >
          ...
          <p className="font-body text-sm text-maroon/50">{t('wishReceived')}</p>
        </motion.div>
      ) : (
        <>
          <div className="flex items-center gap-2">
            <span className="font-body text-xs text-maroon/50">{t('wishIn')}</span>
            <button
              type="button"
          ...
          <div>
            <label htmlFor="wish-name" className="font-body text-xs font-medium text-maroon/60">
              {t('yourName')} *
            </label>
            <input
              id="wish-name"
          ...
          <div>
            <label htmlFor="wish-message" className="font-body text-xs font-medium text-maroon/60">
              {t('yourWish')} *
            </label>
            <textarea
              id="wish-message"
            ...
            />
            <p className="mt-1 text-right font-body text-xs text-maroon/40">{message.length}/280</p>
          </div>
    ...
    <section id="wishes" className="section-padding mandala-bg">
      <div className="section-container">
        <AnimatedSection className="mb-12 text-center" stagger>
          <motion.div variants={fadeInUp} className="text-4xl" aria-hidden="true">
            🙏
          </motion.div>
          <motion.h2 variants={fadeInUp} className="section-title mt-3 text-gold">
            {t('title')}
          </motion.h2>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_2fr]">
          ...
          <div>
            {error !== null && <p className="mb-4 font-body text-sm text-red-400">{error}</p>}
